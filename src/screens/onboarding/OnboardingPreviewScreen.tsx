...
export const OnboardingPreviewScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const fadeAnims = useRef(CHECKLIST.map(() => new Animated.Value(0))).current;
  const slideAnims = useRef(CHECKLIST.map(() => new Animated.Value(15))).current;

  useEffect(() => {
    const animations = CHECKLIST.map((_, i) =>
      Animated.parallel([
    ...
    // Slight delay before stagger starts so the screen has time to mount
    setTimeout(() => {
    ...
    <View style={[styles.container, { backgroundColor: colors.background, paddingTop: insets.top }]}>
      <View style={styles.inner}>
        ...
        <View style={[styles.checklist, { backgroundColor: colors.surface }]}>
          <Text style={[styles.checklistTitle, { color: colors.subtext }]}>
            {(() => {
              try {
                const hour = parseInt(previewAlarm.time.split(':')[0], 10);
                if (hour >= 5 && hour < 12) return 'MORNING PROTOCOL';
                if (hour >= 12 && hour < 17) return 'AFTERNOON PROTOCOL';
                if (hour >= 17 && hour < 21) return 'EVENING PROTOCOL';
                return 'NIGHT PROTOCOL';
              } catch {
                return 'AWAKENING PROTOCOL';
              }
            })()}
          </Text>
